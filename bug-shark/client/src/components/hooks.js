...
const useAuthLogic = (user) => {
    ...
    const fetchUser = async (user) => {
        try {
            ...
            const fetchUser = await fetch(`${process.env.REACT_APP_API_ROOT}/users`, {
                method: "POST",
...
const useNavProfileRequest = (user_id) => {
    ...
    const fetchProfile = async (user_id) => {
        try {
            const fetchProfile = await fetch(`${process.env.REACT_APP_API_ROOT}/users/${user_id}`);
...
const useProjectRequest = (project_id) => {
    ...
    const getProject = async (project_id) => {
        try {
            const fetchedProject = await fetch(`${process.env.REACT_APP_API_ROOT}/projects/${project_id}`);
